...
public class InputFrame extends Button{
	...
	public void init() {
		...
		for(int i = (int) (y + BORDER_SIZE); i < y + l; i += BUTTON_SIZE + (BORDER_SIZE * 2)) {
			buttons[index] = new Button("", "decreases value of parameter " + inputNames[index1], x + BORDER_SIZE, i, BUTTON_SIZE, false){
				public void click(PApplet p, Engine e) {
					
				}
			};
			if (index < buttons.length) index++;
			buttons[index] = new Button("", "increases value of parameter " + inputNames[index1], x + BORDER_SIZE + (BUTTON_SIZE * 2), i, BUTTON_SIZE, false){
				public void click(PApplet p, Engine e) {
